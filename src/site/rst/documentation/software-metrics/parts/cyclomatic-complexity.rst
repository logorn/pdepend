...
was going on in this method. ::
  ...
  {
      $callT  = array(
          \PHP\Depend\Tokenizer::T_STRING,
          \PHP\Depend\Tokenizer::T_VARIABLE
      );
      $chainT = array(
          \PHP\Depend\Tokenizer::T_DOUBLE_COLON,
          \PHP\Depend\Tokenizer::T_OBJECT_OPERATOR,
      );
      ...
      for ($i = 0; $i < $count; ++$i) {
          ...
          if ($tokens[$i]->type === \PHP\Depend\Tokenizer::T_CURLY_BRACE_OPEN) {
              break;
          }
      }
      ...
      for (; $i < $count; ++$i) {
          ...
          if ($tokens[$i]->type !== \PHP\Depend\Tokenizer::T_PARENTHESIS_OPEN) {
              continue;
          }
          // Skip first token
          ...
          } else if ($tokens[$i - 2]->type !== \PHP\Depend\Tokenizer::T_NEW
